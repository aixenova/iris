...
For the two-sheet follow-up sync, keep:

- `Extension data!A`: patient ID captured by the extension
- `Extension data!C`: selected follow-up days
- `Extension data!E`: captured date/time
- `All Contacts!A`: exact UID match against `Extension data!A`
- `All Contacts!C`: embedded UID match against `Extension data!A`
- `All Contacts!I`: destination patient visit date
- `All Contacts!J`: destination selected days
- `All Contacts!K` and `All Contacts!L`: follow-up formula columns
...
6. Click `Sync all rows` in the extension popup to reconcile all existing `Extension data` rows into matching `All Contacts` rows

### 6. Tomorrow follow-up tab and email

The script can rebuild a `Tomorrow Follow-ups` tab from `All Contacts` by calculating real dates from `Patient visit Date` and `Day`.

Use this URL after deployment to rebuild the tab manually:

`YOUR_WEB_APP_URL?action=refreshTomorrowFollowUps`

Use this URL to export the current `Tomorrow Follow-ups` tab as a CSV that opens in Excel:

`YOUR_WEB_APP_URL?action=exportTomorrowFollowUpsCsv`

To email the tab as a CSV attachment, set `FOLLOWUP_EMAIL_RECIPIENTS` in the Apps Script code and use:

`YOUR_WEB_APP_URL?action=emailTomorrowFollowUps`

Run `installThreeHourTomorrowFollowUpsTrigger` once from the Apps Script editor to rebuild and email every 3 hours. If `FOLLOWUP_EMAIL_RECIPIENTS` is blank, the trigger rebuilds the tab but skips email.

Use this URL to remove the three-hour trigger:

`YOUR_WEB_APP_URL?action=removeThreeHourTomorrowFollowUpsTrigger`

### 7. Optional 30-minute background sync

After pasting the Apps Script, run `installThirtyMinuteSyncTrigger` once from the Apps Script editor to install the 30-minute time trigger. You can also try this deployed web app URL after deployment:

`YOUR_WEB_APP_URL?action=installThirtyMinuteSyncTrigger`

Run `removeThirtyMinuteSyncTrigger` from the Apps Script editor, or use this URL, to remove the scheduled trigger:

`YOUR_WEB_APP_URL?action=removeThirtyMinuteSyncTrigger`

To remove accidental blank rows or `CODEX_TEST` rows from `Extension data`, use:

`YOUR_WEB_APP_URL?action=cleanupTestRows`
